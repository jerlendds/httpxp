When making a request over HTTPS, HTTPXP needs to verify the identity of the requested host. To do this, it uses a bundle of SSL certificates (a.k.a. CA bundle) delivered by a trusted certificate authority (CA).

### Enabling and disabling verification

By default httpxp will verify HTTPS connections, and raise an error for invalid SSL cases...

```pycon
>>> httpxp.get("https://expired.badssl.com/")
httpxp.ConnectError: [SSL: CERTIFICATE_VERIFY_FAILED] certificate verify failed: certificate has expired (_ssl.c:997)
```

You can disable SSL verification completely and allow insecure requests...

```pycon
>>> httpxp.get("https://expired.badssl.com/", verify=False)
<Response [200 OK]>
```
...
client = httpxp.Client(verify=ctx)
```
...
`httpxp` does respect the `SSL_CERT_FILE` and `SSL_CERT_DIR` environment variables by default. For details, refer to [the section on the environment variables page](../environment_variables.md#ssl_cert_file).
...
3. Configure `httpxp` to use the certificates stored in `client.pem`.
