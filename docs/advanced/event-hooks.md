HTTPXP allows you to register "event hooks" with the client, that are called
...
client = httpxp.Client(event_hooks={'request': [log_request], 'response': [log_response]})
```

You can also use these hooks to install response processing code, such as this
example, which creates a client instance that always raises `httpxp.HTTPStatusError`
...
client = httpxp.Client(event_hooks={'response': [raise_on_4xx_5xx]})
```
...
client = httpxp.Client(event_hooks={'request': [add_timestamp]})
```
...
!!! note
    If you are using HTTPXP's async support, then you need to be aware that
    hooks registered with `httpxp.AsyncClient` MUST be async functions,
