...
argument on the client. It takes instances of `httpxp.Limits` which define:
...
```python
limits = httpxp.Limits(max_keepalive_connections=5, max_connections=10)
client = httpxp.Client(limits=limits)
```