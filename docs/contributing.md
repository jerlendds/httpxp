...
[proxy section](https://github.com/jerlendds/httpxp/blob/master/docs/advanced/proxies.md#http-proxies) and
the [SSL certificates section](https://github.com/jerlendds/httpxp/blob/master/docs/advanced/ssl.md),
this is where our previously generated `client.pem` comes in:

```python
ctx = ssl.create_default_context(cafile="/path/to/client.pem")
client = httpxp.Client(proxy="http://127.0.0.1:8080/", verify=ctx)
```
