...
def test_headers():
    h = httpxp.Headers([("a", "123"), ("a", "456"), ("b", "789")])
    assert "a" in h
    assert "A" in h
    assert "b" in h
    assert "B" in h
...
def test_copy_headers_method():
    headers = httpxp.Headers({"custom": "example"})
...
def test_copy_headers_init():
    headers = httpxp.Headers({"custom": "example"})
    headers_copy = httpxp.Headers(headers)
    assert headers == headers_copy


def test_headers_insert_retains_ordering():
    headers = httpxp.Headers({"a": "a", "b": "b", "c": "c"})
    headers["b"] = "123"
...
def test_headers_insert_removes_all_existing():
    headers = httpxp.Headers([("a", "123"), ("a", "456")])
    headers["a"] = "789"
...
def test_headers_encoding_in_repr():
    ...
    """
    headers = httpxp.Headers({b"custom": "example ☃".encode("utf-8")})
...
def test_headers_list_repr():
    ...
    """
    headers = httpxp.Headers([("custom", "example 1"), ("custom", "example 2")])
    assert (
...
def test_headers_decode_ascii():
    ...
    headers = httpxp.Headers(raw_headers)
...
def test_multiple_headers():
    ...
    """
    h = httpxp.Headers([("set-cookie", "a, b"), ("set-cookie", "c")])
    assert h.get_list("Set-Cookie") == ["a, b", "c"]

    h = httpxp.Headers([("vary", "a, b"), ("vary", "c")])
...
def test_obfuscate_sensitive_headers(headers, output):
    ...
    headers_class = httpxp.Headers({k: v for k, v in headers})
...
def test_parse_header_links(value, expected):
    all_links = httpxp.Response(200, headers={"link": value}).links.values()
    assert all(link in all_links for link in expected)


def test_parse_header_links_no_link():
    all_links = httpxp.Response(200).links
    assert all_links == {}
