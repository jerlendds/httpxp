...
def test_request_repr():
    request = httpxp.Request("GET", "http://example.org")
...
def test_content_length_header():
    request = httpxp.Request("POST", "http://example.org", content=b"test 123")
...
def test_iterable_content():
    ...
    request = httpxp.Request("POST", "http://example.org", content=Content())
...
def test_url_encoded_data():
    request = httpxp.Request("POST", "http://example.org", data={"test": "123"})
    request.read()
...
def test_json_encoded_data():
    request = httpxp.Request("POST", "http://example.org", json={"test": 123})
    request.read()
...
def test_cannot_access_streaming_content_without_read():
    ...
    request = httpxp.Request("POST", "http://example.org", content=streaming_body())
    with pytest.raises(httpxp.RequestNotRead):
...
def test_transfer_encoding_header():
    ...
    request = httpxp.Request("POST", "http://example.org", content=data)
    assert "Content-Length" not in request.headers
...
def test_override_host_header():
    ...
    request = httpxp.Request("GET", "http://example.org", headers=headers)
...
def test_url():
    ...
    request = httpxp.Request("GET", url)
...
def test_request_params():
    request = httpxp.Request("GET", "http://example.com", params={})
    ...
    request = httpxp.Request("GET", "http://example.com?a=1", params={})
    assert str(request.url) == "http://example.com"
