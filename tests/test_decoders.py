...
def test_zstd_empty():
    headers = [(b"Content-Encoding", b"zstd")]
    response = httpxp.Response(200, headers=headers, content=b"")
...
def test_zstd_multiframe():
    ...
    headers = [(b"Content-Encoding", b"zstd")]
    response = httpxp.Response(200, headers=headers, content=compressed_body)
    response.read()
...
def test_decoders_empty_cases(header_value):
    headers = [(b"Content-Encoding", header_value)]
    response = httpxp.Response(content=b"", status_code=200, headers=headers)
...
def test_decoding_errors(header_value):
    ...
    with pytest.raises(httpxp.DecodingError):
        ...
        httpxp.Response(200, headers=headers, content=compressed_body, request=request)

    with pytest.raises(httpxp.DecodingError):
        httpxp.Response(200, headers=headers, content=compressed_body)
...
async def test_text_decoder_with_autodetect(data, encoding):
    ...
    response = httpxp.Response(200, content=iterator(), default_encoding=autodetect)
...
def test_streaming_text_decoder(
    ...
    response = httpxp.Response(200, content=iter(data))
...
def test_line_decoder_nl():
    ...
    response = httpxp.Response(200, content=[b"", b"a\n\nb\nc"])
...
def test_line_decoder_cr():
    response = httpxp.Response(200, content=[b"", b"a\r\rb\rc"])
    assert list(response.iter_lines()) == ["a", "", "b", "c"]

    response = httpxp.Response(200, content=[b"", b"a\r\rb\rc\r"])
    assert list(response.iter_lines()) == ["a", "", "b", "c"]
...
def test_line_decoder_crnl():
    ...
    response = httpxp.Response(200, content=[b"", b"a\r", b"\n\r\nb\r\nc"])
    assert list(response.iter_lines()) == ["a", "", "b", "c"]

    # Issue #1033
    response = httpxp.Response(200, content=[b"", b"12345\r\n", b"foo bar baz\r\n"])
    assert list(response.iter_lines()) == ["12345", "foo bar baz"]
