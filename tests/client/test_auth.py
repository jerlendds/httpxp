...
class App:
    ...
    def __call__(self, request: httpxp.Request) -> httpxp.Response:
        headers = {"www-authenticate": self.auth_header} if self.auth_header else {}
        data = {"auth": request.headers.get("Authorization")}
        return httpxp.Response(self.status_code, headers=headers, json=data)
...
class DigestApp:
    ...
    def challenge_send(self, request: httpxp.Request) -> httpxp.Response:
        ...
        headers = {
            "www-authenticate": f'Digest realm="httpxp@example.org", {challenge_str}',
        }
        return httpxp.Response(401, headers=headers)


class RepeatAuth(httpxp.Auth):
    """
    ...
    def auth_flow(
        self, request: httpxp.Request
    ) -> typing.Generator[httpxp.Request, httpxp.Response, None]:
        nonces = []
...
class ResponseBodyAuth(httpxp.Auth):
    """
...
class SyncOrAsyncAuth(httpxp.Auth):
    """
    ...
    async def async_auth_flow(
        self, request: httpxp.Request
    ) -> typing.AsyncGenerator[httpxp.Request, httpxp.Response]:
...
async def test_basic_auth() -> None:
    ...
    async with httpxp.AsyncClient(transport=httpxp.MockTransport(app)) as client:
...
async def test_basic_auth_with_stream() -> None:
    ...
    async with httpxp.AsyncClient(
        transport=httpxp.MockTransport(app), auth=auth
    ) as client:
...
async def test_custom_auth() -> None:
    ...
    def auth(request: httpxp.Request) -> httpxp.Request:
...
def test_netrc_auth_credentials_exist() -> None:
    ...
    app = App()
    auth = httpxp.NetRCAuth(netrc_file)

    with httpxp.Client(transport=httpxp.MockTransport(app), auth=auth) as client:
...
def test_auth_hidden_url() -> None:
    ...
    assert url == httpxp.URL(url)
    assert expected == repr(httpxp.URL(url))
...
async def test_auth_property() -> None:
    ...
    async with httpxp.AsyncClient(transport=httpxp.MockTransport(app)) as client:
        ...
        assert isinstance(client.auth, httpxp.BasicAuth)
...
async def test_digest_auth_returns_no_auth_if_no_digest_header_in_response() -> None:
    url = "https://example.org/"
    auth = httpxp.DigestAuth(username="user", password="password123")
    app = App()
...
def test_digest_auth_returns_no_auth_if_alternate_auth_scheme() -> None:
    ...
    client = httpxp.Client(transport=httpxp.MockTransport(app))
...
async def test_digest_auth(
    ...
    assert digest_data["realm"] == '"httpxp@example.org"'
...
async def test_digest_auth_qop_must_be_auth_or_auth_int() -> None:
    ...
    async with httpxp.AsyncClient(transport=httpxp.MockTransport(app)) as client:
        with pytest.raises(httpxp.ProtocolError):
            await client.get(url, auth=auth)
...
@pytest.mark.parametrize(
    "auth_header",
    [
        'Digest realm="httpxp@example.org", qop="auth"',  # missing fields
        'Digest realm="httpxp@example.org", qop="auth,au',  # malformed fields list
    ],
)
@pytest.mark.anyio
...
def test_sync_digest_auth_raises_protocol_error_on_malformed_header(
    ...
    with httpxp.Client(transport=httpxp.MockTransport(app)) as client:
        with pytest.raises(httpxp.ProtocolError):
...
class ConsumeBodyTransport(httpxp.MockTransport):
    async def handle_async_request(self, request: httpxp.Request) -> httpxp.Response:
...
async def test_digest_auth_unavailable_streaming_body():
    ...
    async with httpxp.AsyncClient(transport=ConsumeBodyTransport(app)) as client:
