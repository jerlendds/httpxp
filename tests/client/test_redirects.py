...
def redirects(request: httpxp.Request) -> httpxp.Response:
    if request.url.scheme not in ("http", "https"):
        raise httpxp.UnsupportedProtocol(
            f"Scheme {request.url.scheme!r} not supported."
        )

    if request.url.path == "/redirect_301":
        status_code = httpxp.codes.MOVED_PERMANENTLY
        content = b"<a href='https://example.org/'>here</a>"
        headers = {"location": "https://example.org/"}
        return httpxp.Response(status_code, headers=headers, content=content)

    elif request.url.path == "/redirect_302":
        status_code = httpxp.codes.FOUND
        headers = {"location": "https://example.org/"}
        return httpxp.Response(status_code, headers=headers)

    elif request.url.path == "/redirect_303":
        status_code = httpxp.codes.SEE_OTHER
        headers = {"location": "https://example.org/"}
        return httpxp.Response(status_code, headers=headers)
        ...
        return httpxp.Response(status_code, headers=raw_headers)
        ...
        params = httpxp.QueryParams(request.url.query)
        count = int(params.get("count", "0"))
        redirect_count = count - 1
        status_code = httpxp.codes.SEE_OTHER if count else httpxp.codes.OK
        if count:
    ...
    if request.url.path == "/redirect_loop":
        ...
        status_code = httpxp.codes.OK
        data = {
            "body": request.content.decode("ascii"),
            "headers": dict(request.headers),
        }
        return httpxp.Response(status_code, json=data)

    elif request.url.path == "/redirect_body":
        status_code = httpxp.codes.PERMANENT_REDIRECT
    ...
    if request.method == "HEAD":
        return httpxp.Response(200)

    return httpxp.Response(200, html="<html><body>Hello, world!</body></html>")


def test_redirect_301():
    client = httpxp.Client(transport=httpxp.MockTransport(redirects))
    response = client.post("https://example.org/redirect_301", follow_redirects=True)
    assert response.status_code == httpxp.codes.OK
...
def test_next_request():
    ...
    assert response.status_code == httpxp.codes.SEE_OTHER
...
async def test_async_next_request():
    async with httpxp.AsyncClient(transport=httpxp.MockTransport(redirects)) as client:
        request = client.build_request("POST", "https://example.org/redirect_303")
...
def test_invalid_redirect():
    client = httpxp.Client(transport=httpxp.MockTransport(redirects))
    with pytest.raises(httpxp.RemoteProtocolError):
...
async def test_async_too_many_redirects():
    async with httpxp.AsyncClient(transport=httpxp.MockTransport(redirects)) as client:
        with pytest.raises(httpxp.TooManyRedirects):
            await client.get(
...
def test_can_stream_if_no_redirect():
    ...
    with client.stream("GET", url, follow_redirects=False) as response:
        pass
    assert response.status_code == httpxp.codes.MOVED_PERMANENTLY
    assert response.headers["location"] == "https://example.org/"


class ConsumeBodyTransport(httpxp.MockTransport):
    def handle_request(self, request: httpxp.Request) -> httpxp.Response:
...
def test_cannot_redirect_streaming_body():
    client = httpxp.Client(transport=ConsumeBodyTransport(redirects))
    url = "https://example.org/redirect_body"
...
def cookie_sessions(request: httpxp.Request) -> httpxp.Response:
    if request.url.path == "/":
...
def test_redirect_cookie_behavior():
    client = httpxp.Client(
        transport=httpxp.MockTransport(cookie_sessions), follow_redirects=True
    )
...
def test_redirect_custom_scheme():
    client = httpxp.Client(transport=httpxp.MockTransport(redirects))
    with pytest.raises(httpxp.UnsupportedProtocol) as e:
