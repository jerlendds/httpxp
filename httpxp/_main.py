...
def print_help() -> None:
    ...
    console.print("[bold]HTTPXP :butterfly:", justify="center")
    console.print()
    console.print("A next generation HTTP client.", justify="center")
    console.print()
    console.print(
        "Usage: [bold]httpxp[/bold] [cyan]<URL> [OPTIONS][/cyan] ", justify="left"
    )
    console.print()
...
def show_request_headers(request: Request) -> None:
    rich.console.Console().print(
        f"* Sending request to {str(request.url)!r} using wreq"
    )
...
def main(
    ...
    try:
        with Client(
            ...
            verify=verify,
            event_hooks={
                "response": [show_response_headers],
                "request": [show_request_headers] if verbose else [],
            },
        ) as client:
            with client.stream(
                method,
